function [xsol, violation] = admm_bregman(problem, params)
% solve the distributed problem
    ...
    for j = 1:max_iter
        ...
        [Xopt, ~] = solve_local_NLPs(problem, X, Lambda, rho);
        % dual variable update
        Lambda = Lambda + rho*build_primal_residual(A,Xopt);
        
        [bool, violation(i)] = check_terminal_condition(Xopt, A, tol);
%         if bool
%             xsol = Xopt;
%             return;
%         else
...
%         
        ...
        violation(i) = primal_residual;
        primal_residual = norm(primal_residual,2);
        if (step_length< tol)||bool
            xsol = Xopt;
            return;
        else
            fprintf('norm of violation = %f\n', violation(i));
            plot(log10(i),log10(violation(i)),'.','MarkerEdgeColor','b')
...
% solve decoupled NLPs for all regions
    ...
    for i = 1 : N_regions
		...
		objective      = @(x)build_objective(x, fi(x), grad_i(x), Lambda, Ai, rho, bregman_distance(abs(x),abs(xi)), bregman_grad(abs(x),abs(xi)));
        opts.HessFcn   = @(x,kappa)(hess_i(x,0,0)+rho*bregman_hess(abs(x),abs(xi))/2);
		nonlcon = @(x)build_nonlcon(x, gi, hi, JJac_i);
		[xk, fval, flag, out, multiplier] = fmincon(objective, xi, [], [], [], [], lbx, ubx, nonlcon, opts);
        X{i}     = xk;
        % dual upgrate
...
function [fun, grad] = build_objective(xi, f, dfdx, lambda, Ai, rho, d_bregman, grad_bregman)
	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % the code assumes that Sigma is symmetric and positive definite!!!
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    fun = build_cost(xi, f, lambda, Ai) + rho*d_bregman/2;
    if nargout > 1
        grad = build_grad(dfdx, lambda, Ai) + rho*grad_bregman/2;
    end
end
...
function d_bregman =  bregman_distance(x,y)
    d_bregman = sum(x .* log(x ./ y) - x+ y);
end

function grad_bregman =  bregman_grad(x,y)
    grad_bregman = log(x) -log(y);
end

function hess_bregman =  bregman_hess(x,y)
    hess_bregman = diag(1./x);
end
