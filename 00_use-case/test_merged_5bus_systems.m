...
% gx3 = problem.locFuns.ggi{3}(x3_test);
 ...
 hx1 = problem.locFuns.hhi{1}(x1);
 hx2 = problem.locFuns.hhi{2}(x2);

consensus = problem.AA{1}*x1_test + problem.AA{2}*x2_test;
%% ALADIN-alpha
...
for i = 1 : length(problem.AA)
    local_funs = originalFuns(problem.locFuns.ffi{i}, problem.sens.gg{i}, problem.sens.HH{i}, problem.AA{i}, [], [], problem.locFuns.ggi{i}, problem.sens.JJac{i}, [], []);
