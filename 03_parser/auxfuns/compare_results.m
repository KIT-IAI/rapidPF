function [tab,dxx, error] = compare_results(x, y)
% compare_results
%
...
%   See also: [run_case_file_splitter](run_case_file_splitter.md)
    ...
    d_qg = cell2mat(arrayfun(@(i)norm(x{i}(:,4) - y{i}(:,4), Inf), 1:numel(x), 'UniformOutput', false))';
    regions = [1:numel(x)]';
    ...
    error = [max(d_ang);max(d_mag);max(d_pg);max(d_qg)];
    dxx = max(dx);
end
