function [ solADM ] = run_ADMM_alex(sProb, opts)
...
%     HQP  = V*diag(e)*transpose(V);
    ...
    % solve QP
    [x, ~] = solveQP(HQP,hQP,AQP,bQP,'sparseBs');
...
%    x2 = (x+y)/2;
    ...
    if opts.rhoUpdate == true
        ...
        if norm(A*x,inf) > 0.9*norm(A*xOld,inf) && i > 1
            opts.rho = opts.rho*1.025;
       % elseif norm(A*xOld,inf) > 1.1*norm(A*x,inf)  && i > 1
       %     opts.rho = opts.rho/1.025;
        end
    end
