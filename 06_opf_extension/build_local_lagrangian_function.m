...
%   dL = grad_f + \sum lambda_i (Jac_g)'(:, i) + \sum mu_i(Jac_h)'(:, i)
    ...
    if nargin == 4
        L = @(x, kappa) f(x) + kappa(1:length(g(x)))'*g(x);
        dLdx = @(x, kappa) df(x) ...
            + get_grad_lambda_g(kappa(1:length(g(x))), dg);
        d2Ldx = @(x, kappa, rho)  get_Hess(dLdx, x, kappa);
    elseif nargin == 6
        L = @(x, kappa) f(x) + kappa(1:length(g(x)))'*g(x) + kappa(length(g(x))+1:end)'*h(x);
        dLdx = @(x, kappa) df(x)  + ...
            get_grad_lambda_g(kappa(1:length(g(x))), dg, x) + ...
            get_grad_mu_h(kappa(length(g(x))+1:end), dh, x);
        d2Ldx = @(x, kappa, rho) get_Hess(dLdx, x, kappa);
    end
