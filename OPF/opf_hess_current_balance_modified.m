function d2G = opf_hess_current_balance_modified(x, entries_pf, lambda,Ncopy, mpc, Ybus, mpopt)
...
nlam = fix(length(lambda) / 2);
lamP = lambda(1:nlam);
lamP = [lamP;zeros(Ncopy,1)];
lamQ = lambda((1:nlam)+nlam);
lamQ = [lamQ;zeros(Ncopy,1)];
